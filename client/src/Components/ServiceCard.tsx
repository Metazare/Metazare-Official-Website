...
function ServiceCard({title,description,variant}:Props) {
      ...
      <Box flexGrow={1}  display={"flex"} justifyContent={"end"}  >
        <Box sx={{maxWidth:{md:"700px",xs:"unset"}}} >
          ...
          <Box display="flex" flexWrap={"wrap"} gap={1} mt={1}>
            ...
            <Chip label="Mongo DB" variant="outlined" />
            <Chip label="Mongo DB" variant="outlined" />
          </Box>
        </Box>
      </Box>
      {/* <Box sx={{width:{md:"400px",xs:"100%"}}} display={"flex"} alignItems={"center"} justifyContent={"center"}>
        <Paper variant="elevation" className='ServiceCardImage' elevation={3}  sx={{background:"#D2D2D2",height:"300px",width:"340px"}}>
        </Paper>
      </Box> */}
    </Box>
  )
