...
interface Props{
  data: any
  modalClose:  () => void
  editFunc: (data: TeamType, id: any) => void
}
export default function EditTeam({data,editFunc,modalClose}:Props) {
    ...
    onSubmit:(values)=>{
      switch (formPage) {
        ...
        case 3:
          ...
          break
        case 4:
          editFunc({...values,image:downloadURL},data.id)
          modalClose();
          break;
        default:
          break;
  ...
  useEffect(()=>{
    ...
    formik.setFieldValue("roles",data.roles)
    formik.setFieldValue("description",data.description)
    formik.setFieldValue("gmail",data.gmail)
    formik.setFieldValue("github",data.github)
            ...
            <Box display="flex" flexDirection={"column"}  height={"300px"} justifyContent={"center"} alignItems={"center"} sx={{border:"1px dashed #1976d2",borderRadius:"8px",gap:".5em",cursor:"pointer",transition:"all .3s ease-in",opacity:".6",":hover":{opacity:"1"}}}>
              {downloadURL? 
                <>
                  ...
                  <Box display="flex">
                    <Avatar variant="circular" src={data.image}  sx={{ width: '200px', height: '200px' }} />
                  </Box>
                  <Typography variant="subtitle2" textAlign={"center"} fontSize={"10px"} color="initial">Click again if you want to change</Typography>
                </>}
            </Box>            
